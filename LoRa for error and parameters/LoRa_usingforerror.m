...
function [BER_nCOH_AWGN_Theory,PER_nCOH_AWGN_Theory ,BER_nCOH_AWGN_SIMULATION,PER_nCOH_AWGN_SIMULATION]=LoraErrorr(SF,BW,SNR,CR,Data_rate)
%% Packet Size
% Symbols
Nm=10;
Rs=Data_rate*((4+CR)/4)*(1/SF);
Rs=round(Rs);
Ns =50*Rs;
% Symbols per packet
%Nm = 1e1 ;% bir paketteki sembol sayısı
N = Ns/Nm; % packet sayısı
...
%     yCoh2 = idx - 1 ;
    %% BER Simulation
    BER_nCOH_AWGN_SIMULATION = sum(sum(abs(de2bi(ynCoh1,M) - de2bi(x,M))))/(SF.*Ns) ;
   % BER_nCOH_RAY_SIMULATION(CtrSNR)  = sum(sum(abs(de2bi(ynCoh2,M) - de2bi(x,M))))/(SF.*Ns) ;
    ...
    %% SER Simulation
    SER_nCOH_AWGN_SIMULATION = sum(abs(ynCoh1 - x)>0)/Ns ;
    ...
    %% PER Simulation
    PER_nCOH_AWGN_SIMULATION = sum(sum(abs(reshape(x,N,Nm) - reshape(ynCoh1,N,Nm)),2)>0)/N ;
